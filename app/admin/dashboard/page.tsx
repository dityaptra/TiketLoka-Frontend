    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Ringkasan performa penjualan tiket.</p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white p-2 rounded-xl border border-gray-300">
          ...
          {(startDate || endDate) && (
            <button onClick={resetFilter} className="p-1 hover:bg-gray-100 rounded-full text-red-500 transition"><X size={16} /></button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard label="Total Pendapatan" value={`Rp ${Number(stats.total_revenue).toLocaleString("id-ID")}`} icon={<DollarSign size={24} className="text-white" />} bgClass="bg-green-500" />
        <StatCard label="Total Transaksi" value={stats.total_bookings} icon={<ShoppingBag size={24} className="text-white" />} bgClass="bg-blue-500" />
        <StatCard label="Tiket Terjual" value={stats.total_tickets_sold} icon={<Ticket size={24} className="text-white" />} bgClass="bg-[#F57C00]" />
        <StatCard label="Total User" value={stats.total_users} icon={<Users size={24} className="text-white" />} bgClass="bg-purple-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.chart_revenue}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "#fff", borderRadius: "8px", border: "1px solid #E5E7EB" }} formatter={(val: number) => [`Rp ${val.toLocaleString("id-ID")}`, "Pendapatan"]} />
                <Line type="monotone" dataKey="total" stroke="#0B2F5E" strokeWidth={3} dot={{ r: 4, fill: "#0B2F5E", strokeWidth: 2, stroke: "#fff" }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-300 shadow-sm">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="font-bold text-gray-800">Wisata Terpopuler (Top 5)</h3>
          </div>
          <div className="h-[300px] w-full">
            {stats.chart_popular.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.chart_popular} layout="vertical" margin={{ left: 20 }}>
                  ...
                  <Bar dataKey="total" fill="#F57C00" radius={[0, 4, 4, 0]} barSize={20} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-gray-400 text-sm">Belum ada data penjualan tiket.</div>
            )}
          </div>
        </div>
      </div>
...
function StatCard({ label, value, icon, bgClass }: StatCardProps) {
  return (
    <div className={`p-5 rounded-xl transition-all duration-300 flex items-center justify-between ${bgClass}`}>
      ...
      <div className="p-3 rounded-lg bg-white/20 backdrop-blur-sm border border-white/10">{icon}</div>
    </div>
  );
}