...
export default function AdminBookings() {
  // Kita tidak butuh "token" string lagi karena sudah otomatis via Cookie
  ...
  // --- FUNGSI FETCH DENGAN AXIOS ---
  const fetchBookings = useCallback(async () => {
    setLoading(true);
    try {
      // Menggunakan Axios: Tidak perlu susun URL manual & tidak perlu Header Auth manual
      ...
      // Axios membungkus respon di properti .data
      ...
      // Jika ingin notifikasi khusus:
      if (err.response?.status === 404) {
         setBookings([]); // Kosongkan jika not found
      }
    } finally {
      setLoading(false);
    }
  }, [page, startDate, endDate, search, status]); // Token dihapus dari dependency
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-white p-2 rounded-xl shadow-sm border border-gray-200">
            ...
            <button
              ...
              className="p-1 text-gray-400 hover:bg-gray-100 rounded-full border-l pl-2 ml-1"
              title="Muat Ulang"
            >
              <RefreshCcw size={14} />
            </button>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-semibold text-gray-800">
                          {item.details?.[0]?.destination?.name || "-"}
                        </td>
                        ...
                        <td className="px-6 py-4 font-black text-gray-600">
                  ...
                  <tr>
                    <td colSpan={8} className="text-center py-20">
                      <div className="flex flex-col items-center opacity-40">
                        ...
                        <button onClick={handleReset} className="text-[#0B2F5E] text-xs underline mt-1">
                          Reset Semua Filter
                        </button>
                      </div>
                    </td>
      ...
      {meta && meta.last_page > 1 && (
        <div className="flex justify-between items-center bg-white p-4 rounded-xl border shadow-sm">
          <span className="text-xs text-gray-400 font-medium">
            Data {meta.from} - {meta.to} dari {meta.total}
          </span>
          <div className="flex gap-1">
            <button
              disabled={page === 1}
              onClick={() => setPage(page - 1)}
              className="px-3 py-1.5 bg-gray-50 rounded-lg text-xs font-bold disabled:opacity-30 border hover:bg-gray-100 text-gray-600 transition-colors"
            >
              Prev
            </button>
  ...
  return (
    <span
      className={`px-2 py-1 rounded-md text-[9px] font-black uppercase border ${
        colors[s] || "bg-gray-100 text-gray-500"
      }`}
    >
      {labels[s] || status}
    </span>
